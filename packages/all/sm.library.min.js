...
"undefined")for(var i=0;i<this._relatesTo.length;i++){if(typeof this._relatesTo[i]._subscribers==="undefined")continue;this.notify(message,this._relatesTo[i]._subscribers)}};var _getGuid=function(c){var r=Math.random()*16|0,v=c=="x"?r:r&3|8;return v.toString(16)};var getGuid=function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,_getGuid)};Channel.prototype.unsubscribe=function(subscriberId){if(typeof this._subscribers!=="undefined")delete this._subscribers[subscriberId]};Channel.prototype.subscribe=
function(subscriber){if(typeof this._subscribers==="undefined")this._subscribers={};var subscriberId=getGuid();if(typeof subscriber==="function")this._subscribers[subscriberId]={update:subscriber,lifetime:1};else{if(typeof subscriber.lifecycle==="undefined")subscriber.lifetime=1;this._subscribers[subscriberId]=subscriber}return subscriberId};Channel.prototype.notify=function(message,subscribers){var authoritative=false,delegates={},delegateCount=0,deference=[];for(var id in subscribers){var response=
subscribers[id].update(message);if(typeof subscribers[id]==="undefined")continue;if(subscribers[id].lifetime<=0)this.unsubscribe(id);if(response===true)authoritative=true;else if(typeof response==="function"){deference.push(response);authoritative=true}else if(response&&typeof response.update==="function"){delegates[getGuid()]=response;delegateCount++}}if(authoritative){var continuedAuthority=false;for(var i=0;i<deference.length;i++){var deferredResult=deference[i](message);if(!deferredResult===false)continuedAuthority=
true}if(continuedAuthority===false)authoritative=false}if(authoritative==false&&delegateCount>0)this.notify(message,delegates)};Channel.prototype.publish=function(message,recipients){var recipients=recipients||{};if(typeof message==="function"){var newResult=message(new sm.type.AsyncResult(this));message=newResult;if(typeof message!=="undefined"&&typeof message.getType==="function"&&message.getType()==="[object AsyncResult]")return this}if(typeof this._subscribers==="undefined"){this.forward(message,
recipients);return this}this.notify(message,this._subscribers);this.forward(message,recipients);return this};Channel.prototype.addHelper=function(name,helper){var method=Channel.prototype.publish;var context=this;this[name]=function(){var args=[];for(var i=0;i<arguments.length;i++)args.push(arguments[i]);return method.apply(context,[function(asyncResult){args.push(asyncResult);return helper.apply(context,args)}])};return this};Channel.prototype.getType=function(){return"[object Channel]"};Channel.prototype.ofType=
function(type){return type==="Channel"||typeof type.getType==="function"&&type.getType()===this.getType()};sm.behavior.extendedBy(Channel,"Channel")})(smallmachine);(function(sm,$){var Extension=function(a){if(typeof $.fn!=="undefined")$.extend({"sm":a});return this};Extension.prototype.initializer=true;sm.behavior.extendedBy(Extension,"Extension")})(smallmachine,jQuery||{});(function(sm){var ontology=new sm.Ontology("sm.nodes");ontology.addTerm("task");ontology.addTerm("insert");ontology.addTerm("connect");ontology.addTerm("set");ontology.addTerm("get");ontology.addTerm("paint");ontology.paint.isA(ontology.task);ontology.insert.isA(ontology.task);ontology.connect.isA(ontology.task);ontology.set.isA(ontology.task);ontology.get.isA(ontology.task);var Node=function(item,paper,shapeAttr){this.id=item.id||function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,
function(c){var r=Math.random()*16|0,v=c=="x"?r:r&3|8;return v.toString(16)})}();this.width=parseFloat(item.width)||50;this.height=parseFloat(item.height)||50;this.y=parseFloat(item.y);this.x=parseFloat(item.x);this.edges=item.edges||[];this.label=item.label||this.id;this.paper=paper;this.shapeAttr=shapeAttr;return this};Node.prototype.update=function(message){var element=this.paper.circle(this.x,this.y,this.width/2,this.height/2);element.id=this.id;if(typeof this.shapeAttr==="function")element.attr(this.shapeAttr(this.label));
else element.attr(this.shapeAttr);this.paper.text(this.x,this.y,this.label);return true};var Edge=function(a,b,index,paper,lineAttr,textLabelAttr){this.id1=a.id;this.id2=b.id;this.x1=parseFloat(a.x);this.y1=parseFloat(a.y);this.x2=parseFloat(b.x);this.y2=parseFloat(b.y);this.label=a.edges[index].label||"";this.paper=paper;this.lineAttr=lineAttr;this.textLabelAttr=textLabelAttr;return this};Edge.prototype.update=function(message){var line=this.paper.path(["M",this.x1,this.y1,"L",this.x2,this.y2]);
if(typeof this.lineAttr==="function")line.attr(this.lineAttr(this.label));else line.attr(this.lineAttr);var a=this.x1-this.x2;var b=this.y1-this.y2;var cX=(this.x1+this.x2)/2;var cY=(this.y1+this.y2)/2;var textLabel=this.paper.text(cX,cY,this.label);if(typeof this.textLabelAttr==="function")textLabel.attr(this.textLabelAttr(this.label));else textLabel.attr(this.textLabelAttr);var dX=this.x2-this.x1;var dY=this.y2-this.y1;var angle=Math.atan2(dY,dX)*180/Math.PI;textLabel.transform("r"+angle);return true};
var getObjectNode=function(message,id){for(var i=0;i<message.length;i++)if(message[i].id===id)return message[i];return null};var PaintDelegate=function(model){this._model=model;this.settings=new sm.type.Config;model.system.get.config(this.settings);this.handleError=function(Error){model.messenger.error.publish(Error)};return this};PaintDelegate.prototype.update=function(message){var json=message.value;if(typeof json==="undefined")return false;if(sm.typeMask(json,{length:true,sort:"function"})!==null)sm.error(new Error("Cannot determine the JSON type; expecting an array (or comparable object) of node objects but received "+
json),this);var paper=new sm.type.NamedValue("sm.raphaeljs","paper",null);this._model.get.publish(paper);var edges=[];for(var i=0;i<json.length;i++){var node=new Node(json[i],paper.value,this.settings.shapeAttr||{});for(var j=0;j<node.edges.length;j++){var objectNode=getObjectNode(json,node.edges[j].id);if(objectNode!==null){var edge=new Edge(node,objectNode,j,paper.value,this.settings.lineAttr||{},this.settings.textLabelAttr||{});this._model.connect.publish(edge)}}this._model.insert.publish(node)}};
var activator=function(model){var delegate=new PaintDelegate(model);model.paint.subscribe(function(message){return delegate});model.connect.subscribe(function(message){return message});model.insert.subscribe(function(message){return message})};ontology.registerActivator(activator,["sm.channels","sm.raphaeljs"]);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.nodes' ontology: "+error.message+"\n"+error.stack))}})(smallmachine);(function(sm,$,raphael){var ontology=new sm.Ontology("sm.flowchart");ontology.addTerm("paint");ontology.addTerm("get");var TOP_MARGIN=50;var SIBLING_MARGIN=20;var SECTOR_MARGIN=40;var GROUP_MARGIN=40;var FIXED_EDGE_LENGTH=false;var Sector=function(previousSector,layoutWidth,layoutHeight,parentId){if(typeof previousSector==="undefined"){this.y=TOP_MARGIN;this.previousSector={y:0,height:0}}else{this.previousSector=previousSector;this.y=this.previousSector.y+this.previousSector.height+SECTOR_MARGIN}this.height=
