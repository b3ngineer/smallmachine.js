...
core.error=function(Error,handler){if(typeof handler!=="undefined"&&typeof handler.handleError==="function")handler.handleError(Error);else if(typeof core.handleError==="function")core.handleError(Error);else throw Error;};core.CONCEPT="concept";core.RELATIONSHIP="relationship";var Proxy=function(Term,Inferencer){this._term=Term;this._rules=Inferencer._rules;return this};Proxy.prototype.getType=function(){return"[object Proxy]"};Proxy.prototype.ofType=function(type){return type==="Proxy"||typeof type.getType===
"function"&&type.getType()===this.getType()};var _Term=function(){return this};_Term.prototype.relate=function(Term){if(typeof this._relatesTo==="undefined")this._relatesTo=[];var alreadyRelatesTo=false;for(var i=0;i<this._relatesTo.length;i++)if(this._relatesTo[i]._id==Term._id){alreadyRelatesTo=true;break}if(!alreadyRelatesTo)this._relatesTo.push(Term)};_Term.prototype.getType=function(){return"[object Term]"};_Term.prototype.ofType=function(type){return type==="Term"||typeof type.getType==="function"&&
type.getType()===this.getType()};core.Behavior=function(title){this.title=title;return this};core.Ontology=function(title){this.title=title;this._inferencer=new Inferencer;this._activators=[];this._term=function(value){this._id=function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(c){var r=Math.random()*16|0,v=c=="x"?r:r&3|8;return v.toString(16)})}();if(typeof value._value!=="undefined"&&typeof value._type!=="undefined"){this._value=value._value;this._type=value._type}else{this._value=
value;this._type=null}return this};return this};core.Ontology.prototype.getType=function(){return"[object Ontology]"};core.Ontology.prototype.addTerm=function(value){if(typeof this[value]!=="undefined")return this;var Term=this._term;Term.prototype.relate=_Term.prototype.relate;Term.prototype.getType=_Term.prototype.getType;Term.prototype.ofType=_Term.prototype.ofType;var t=new Term(value);this[value]=new Proxy(t,this._inferencer);return this};core.Ontology.prototype.registerActivator=function(activator,
dependencies){if(typeof dependencies==="undefined")dependencies=[];if(typeof activator!=="function"&&typeof activator.getType==="function"&&activator.getType()==="[object Activator]"){this._activators.push(activator);return}this._activators.push({fn:activator,title:this.title,dependencies:dependencies,getType:function(){return"[object Activator]"}})};core.Ontology.prototype.getModel=function(behaviors){var Model=function(title){return this};var model=new Model;var allBehaviors=[];if(typeof behaviors!==
"undefined"){allBehaviors=allBehaviors.concat(behaviors);for(var i=0;i<allBehaviors.length;i++)if(typeof allBehaviors[i].prototype!=="undefined"&&allBehaviors[i].prototype.initializer===true)allBehaviors[i](model)}for(var p in this){if(!this.hasOwnProperty(p))continue;if(typeof this[p]==="undefined"||this[p]._term==="undefined")continue;if(p.indexOf("_")===0)continue;if(typeof this[p]._term!=="undefined"){model[p]=this[p]._term;for(var i=0;i<allBehaviors.length;i++)core.alsoBehavesLike(Object.getPrototypeOf(model[p]),
allBehaviors[i].prototype)}else model[p]=this[p]}this._inferencer._rules.sort(function(a,b){return a._priority-b._priority});for(var i=0;i<this._inferencer._rules.length;i++){var rule=this._inferencer._rules[i];rule._fn.apply(model,rule._args)}for(var i=0;i<this._activators.length;i++)this._activators[i].fn(model);return model};var Inferencer=function(){this._term={_value:null};this._rules=[];return this};var Rule=function(priority,fn,args){this._fn=fn;this._args=args;this._priority=priority;return this};
Inferencer.prototype.isA=function(Proxy){this._rules.push(new Rule(1,isA,[this._term._value,Proxy._term._value]));return this};Inferencer.prototype.relatesTo=function(ProxyA,ProxyB){this._rules.push(new Rule(10,relatesTo,[this._term._value,ProxyA._term._value,ProxyB._term._value]));return this};Inferencer.prototype.hasDomain=function(Proxy){this._rules.push(new Rule(3,hasDomain,[this._term._value,Proxy._term._value]));return this};Inferencer.prototype.hasRange=function(Proxy){this._rules.push(new Rule(2,
hasRange,[this._term._value,Proxy._term._value]));return this};core.alsoBehavesLike(Proxy,Inferencer);var relatesTo=function(target,termA,termB){var Term=this[target];var TermA=this[termA];var TermB=this[termB];if(TermA._type===core.CONCEPT)core.error(new Error("Cannot define a relationship with a concept type: "+TermA._value));else if(TermA._type===null)TermA._type=core.RELATIONSHIP;if(TermB._type===core.RELATIONSHIP)core.error(new Error("Cannot create a relationship with a relationship type: "+
TermB._value));else if(TermB._type===null)TermB._type=core.CONCEPT;if(Term._type===core.RELATIONSHIP)core.error(new Error("Cannot create a relationship from a relationship type: "+Term._value));else if(Term._type===null)Term._type=core.CONCEPT;if(typeof Term[termA]==="undefined"){var ctor=Term.constructor;Term[termA]=new ctor(TermA)}if(typeof Term[termA][termB]==="undefined")Term[termA][termB]=TermB;if(typeof Term[termB]==="undefined")Term[termB]=TermB;TermB.relate(Term[termA]);for(var field in TermB){if(!TermB.hasOwnProperty(field))continue;
if(TermB[field].getType&&TermB[field].getType()==="[object Term]"){if(typeof Term[termA][field]==="undefined")Term[termA][field]=TermB[field];if(typeof Term[field]==="undefined")Term[field]=TermB[field];TermB[field].relate(Term[termA])}}};var isA=function(target,termA){var Term=this[target];var TermA=this[termA];if(TermA._type===core.RELATIONSHIP)core.error(new Error("Cannot apply isA to a relationship type: "+TermA._value));else if(TermA._type===null)TermA._type=core.CONCEPT;if(Term._type===core.RELATIONSHIP)core.error(new Error("Cannot apply isA to a relationship type: "+
Term._value));else if(Term._type===null)Term._type=core.CONCEPT;if(typeof TermA[target]==="undefined")TermA[target]=Term};var hasRange=function(target,termA){var Term=this[target];var TermA=this[termA];if(TermA._type===core.RELATIONSHIP)core.error(new Error("Cannot apply hasRange to a relationship type: "+TermA._value));else if(TermA._type===null)TermA._type=core.CONCEPT;if(Term._type===core.CONCEPT)core.error(new Error("Cannot assign hasRange from a concept type: "+Term._value));else if(Term._type===
null)Term._type=core.RELATIONSHIP;if(typeof Term[termA]==="undefined")Term[termA]=TermA;TermA.relate(Term);for(var field in TermA){if(!TermA.hasOwnProperty(field))continue;if(TermA[field].getType&&TermA[field].getType()==="[object Term]"){var term=TermA[field]._value;if(typeof Term[term]==="undefined")Term[term]=TermA[field];TermA[field].relate(Term)}}};var hasDomain=function(target,termA){var Term=this[target];var TermA=this[termA];if(TermA._type===core.RELATIONSIP)core.error(new Error("Cannot apply hasRange to a relationship type: "+
TermA._value));else if(TermA._type===null)TermA._type=core.CONCEPT;if(Term._type===core.CONCEPT)core.error(new Error("Cannot assign hasRange from a concept type: "+Term._value));else if(Term._type===null)Term._type=core.RELATIONSHIP;if(typeof TermA[target]==="undefined")TermA[target]=Term};core.typeMask=function(a,b){var result=[];for(var p in b){if(!b.hasOwnProperty(p))continue;if(b[p]===true){if(typeof a[p]==="undefined")result.push(p)}else if(typeof a[p]!=="undefinedf"&&typeof a[p]!==b[p])result.push(p)}return result.length===
0?null:result};return core}(smallmachine||{});(function(sm){var o=new sm.Ontology("sm.properties");o.addTerm("property");o.addTerm("type");o.addTerm("ontology");o.addTerm("behavior");o.addTerm("hasMemberType");o.addTerm("hasMember");o.addTerm("Ontology");o.addTerm("Behavior");o.addTerm("Constructor");o.addTerm("Null");o.ontology.isA(o.property);o.behavior.isA(o.property);o.Ontology.isA(o.type);o.Behavior.isA(o.type);o.Constructor.isA(o.type);o.Null.isA(o.type);o.ontology.relatesTo(o.hasMemberType,o.Ontology);o.behavior.relatesTo(o.hasMemberType,
...
textStatus,jqxhr){model.messenger.success.publish(textStatus);asyncResult.publish(data)},error:function(jqxhr,textStatus,thrown){sm.error(new Error(thrown))}})}catch(error){sm.error(new Error(error))}else{var xhr=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("MSXML2.XMLHTTP.3.0");xhr.onprogress=function(){};xhr.onerror={};xhr.onreadystatechange=function(){if(xhr.readyState<4)return;if(xhr.status==200){model.messenger.success.publish(xhr.statusText);asyncResult.publish(JSON.parse(xhr.responseText))}else sm.error(new Error(xhr.statusText))};
xhr.open("GET",url,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.setRequestHeader("Content-type","application/json");xhr.send(null)}return asyncResult};var defaultStorageDelegate={update:function(message){if(typeof message.ofType==="function"&&message.ofType("NamedValue"))return function(message){model.memory.modify(message)};return false}};sm.alsoBehavesLike(model,{memory:new sm.type.NamedValueCollection});model.set.subscribe({update:function(message){return defaultStorageDelegate}});
var defaultValueDelegate={update:function(message){if(typeof message.ofType==="function")return function(message){model.memory.getValue(message)};return false}};model.get.subscribe({update:function(message){return defaultValueDelegate}});if(typeof model.initialize.addHelper==="function")model.initialize.addHelper("json",jsonHelper);if(typeof model.set.addHelper==="function")model.set.addHelper("config",function(config){return new sm.type.NamedValue("sm.channels","config",config)});if(typeof model.get.addHelper===
"function"){model.get.addHelper("json",jsonHelper);var Config=function(){this.namespace="sm.channels";this.key="config";return this};Config.prototype.setValue=function(value){sm.alsoBehavesLike(this,value)};sm.type.extendedBy(Config,"Config");model.get.addHelper("config",function(outVar){return outVar})}};ontology.registerActivator(activator);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.channels' ontology: "+
...
"undefined")for(var i=0;i<this._relatesTo.length;i++){if(typeof this._relatesTo[i]._subscribers==="undefined")continue;this.notify(message,this._relatesTo[i]._subscribers)}};Channel.prototype.subscribe=function(subscriber){if(typeof this._subscribers==="undefined")this._subscribers=[];if(typeof subscriber==="function"){this._subscribers.push({update:subscriber});return this}this._subscribers.push(subscriber);return this};Channel.prototype.notify=function(message,subscribers){var authoritative=false,
delegates=[],deference=[];for(var i=0;i<subscribers.length;i++){var response=subscribers[i].update(message);if(response===true)authoritative=true;else if(typeof response==="function"){deference.push(response);authoritative=true}else if(response&&typeof response.update==="function")delegates.push(response)}if(authoritative){var continuedAuthority=false;for(var i=0;i<deference.length;i++){var deferredResult=deference[i](message);if(!deferredResult===false)continuedAuthority=true}if(continuedAuthority===
false)authoritative=false}if(authoritative==false)if(delegates.length>0)this.notify(message,delegates)};Channel.prototype.publish=function(message,recipients){var recipients=recipients||{};if(typeof message==="function"){var newResult=message(new sm.type.AsyncResult(this));message=newResult;if(typeof message!=="undefined"&&typeof message.getType==="function"&&message.getType()==="[object AsyncResult]")return this}if(typeof this._subscribers==="undefined"){this.forward(message,recipients);return this}this.notify(message,
this._subscribers);this.forward(message,recipients);return this};Channel.prototype.addHelper=function(name,helper){var method=Channel.prototype.publish;var context=this;this[name]=function(){var args=[];for(var i=0;i<arguments.length;i++)args.push(arguments[i]);return method.apply(context,[function(asyncResult){args.push(asyncResult);return helper.apply(context,args)}])};return this};Channel.prototype.getType=function(){return"[object Channel]"};Channel.prototype.ofType=function(type){return type===
"Channel"||typeof type.getType==="function"&&type.getType()===this.getType()};sm.behavior.extendedBy(Channel,"Channel")})(smallmachine);(function(sm,$){var Extension=function(a){if(typeof $.fn!=="undefined")$.extend({"sm":a});return this};Extension.prototype.initializer=true;sm.behavior.extendedBy(Extension,"Extension")})(smallmachine,jQuery||{});(function(sm){var ontology=new sm.Ontology("sm.nodes");ontology.addTerm("insert");ontology.addTerm("connect");ontology.addTerm("set");ontology.addTerm("get");ontology.addTerm("initialize");var Node=function(item,paper){this.id=item.id||function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(c){var r=Math.random()*16|0,v=c=="x"?r:r&3|8;return v.toString(16)})}();this.width=parseFloat(item.width)||50;this.height=parseFloat(item.height)||50;this.y=parseFloat(item.y);this.x=parseFloat(item.x);
this.edges=item.edges||[];this.label=item.label||this.id;this.paper=paper;return this};Node.prototype.update=function(message){var element=this.paper.circle(this.x,this.y,this.width/2,this.height/2);element.id=this.id;element.attr({"fill":"90-#777793:5-#FFF:95","stroke":"#FFF","stroke-width":2});this.paper.text(this.x,this.y,this.label);return true};var Edge=function(a,b,index,paper){this.id1=a.id;this.id2=b.id;this.x1=parseFloat(a.x);this.y1=parseFloat(a.y);this.x2=parseFloat(b.x);this.y2=parseFloat(b.y);
this.label=a.edges[index].label||"";this.paper=paper;return this};Edge.prototype.update=function(message){var line=this.paper.path(["M",this.x1,this.y1,"L",this.x2,this.y2]);line.attr({"stroke":"#aab","arrow-end":"classic-wide-long"});var a=this.x1-this.x2;var b=this.y1-this.y2;var cX=(this.x1+this.x2)/2;var cY=(this.y1+this.y2)/2;var textLabel=this.paper.text(cX,cY,this.label);textLabel.attr({"fill":"#eef","font-weight":"normal"});var dX=this.x2-this.x1;var dY=this.y2-this.y1;var angle=Math.atan2(dY,
dX)*180/Math.PI;textLabel.transform("r"+angle);return true};var getObjectNode=function(message,id){for(var i=0;i<message.length;i++)if(message[i].id===id)return message[i];return null};var InitializerDelegate=function(model){this._model=model;this.handleError=function(Error){model.messenger.error.publish(Error)};return this};InitializerDelegate.prototype.update=function(message){if(sm.typeMask(message,{length:true,sort:"function"})!==null)sm.error(new Error("Cannot determine the message type; expecting an array (or comparable object) of node objects but received "+
message),this);var paper=new sm.type.NamedValue("sm.raphaeljs","paper",null);this._model.get.publish(paper);var edges=[];for(var i=0;i<message.length;i++){var node=new Node(message[i],paper.value);for(var j=0;j<node.edges.length;j++){var objectNode=getObjectNode(message,node.edges[j].id);if(objectNode!==null){var edge=new Edge(node,objectNode,j,paper.value);this._model.connect.publish(edge)}}this._model.insert.publish(node)}};var activator=function(model){var delegate=new InitializerDelegate(model);
model.initialize.subscribe(function(message){return delegate});model.connect.subscribe(function(message){return message});model.insert.subscribe(function(message){return message})};ontology.registerActivator(activator,["sm.channels","sm.raphaeljs"]);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.nodes' ontology: "+error.message+"\n"+error.stack))}})(smallmachine);(function(sm,$,raphael){var ontology=new sm.Ontology("sm.flowchart");ontology.addTerm("initialize");ontology.addTerm("get");var TOP_MARGIN=50;var SIBLING_MARGIN=20;var SECTOR_MARGIN=40;var GROUP_MARGIN=40;var FIXED_EDGE_LENGTH=false;var Sector=function(previousSector,layoutWidth,layoutHeight,parentId){if(typeof previousSector==="undefined"){this.y=TOP_MARGIN;this.previousSector={y:0,height:0}}else{this.previousSector=previousSector;this.y=this.previousSector.y+this.previousSector.height+SECTOR_MARGIN}this.height=
0;this.x=layoutWidth*0.5;this.width=0;this.lastNodeGroup=parentId;this.layoutWidth=layoutWidth;this.nodes=[];return this};Sector.prototype.add=function(node,nodeGroup){var nodeWidth=parseFloat(node.width);this.width=this.width+SIBLING_MARGIN+nodeWidth;var nodeHeight=parseFloat(node.height);this.height=this.height<nodeHeight?nodeHeight:this.height;this.x=this.x-SIBLING_MARGIN*0.5-nodeWidth*0.5;if(!FIXED_EDGE_LENGTH){var maxEdgeLabelLength=0;for(var i=0;i<node.edges.length;i++)if(node.edges[i].label.length>
maxEdgeLabelLength)maxEdgeLabelLength=node.edges[i].label.length;if(maxEdgeLabelLength>8){var newHeight=this.height+(maxEdgeLabelLength-8)*10;if(newHeight>this.height)this.height=newHeight}}this.nodes.push({node:node,nodeGroup:nodeGroup})};Sector.prototype.finalize=function(){var lastNodeGroup=null;for(var i=0;i<this.nodes.length;i++){var margin=SIBLING_MARGIN*i;if(lastNodeGroup===null)lastNodeGroup=this.nodes[i].nodeGroup;else if(this.nodes[i].nodeGroup!=lastNodeGroup){margin=margin+GROUP_MARGIN;
lastNodeGroup=this.nodes[i].nodeGroup}this.nodes[i].node.x=this.x+margin+this.nodes[i].node.width*i;this.nodes[i].node.y=this.y}};var Layout=function(paper){this.paper=paper;this.width=parseFloat(paper.value.width);this.height=parseFloat(paper.value.height);this.sectors=[];return this};Layout.prototype.addToSector=function(node,sectorId,parentId){var sector=this.sectors[sectorId];if(typeof sector==="undefined"){sector=new Sector(this.sectors[sectorId-1],this.width,this.height,parentId);this.sectors[sectorId]=
sector}sector.add(node,parentId)};Layout.prototype.finalize=function(){for(var i=0;i<this.sectors.length;i++)this.sectors[i].finalize()};var format=function(nodes,root,layout){var nodeIndex={};var sectorId=0;var rootNode=nodes[0];for(var i=0;i<nodes.length;i++){nodeIndex[nodes[i].id]=i;if(nodes[i].id===root)rootNode=nodes[i]}setNodeCoords(rootNode,nodes,nodeIndex,layout,sectorId,rootNode.id);layout.finalize()};var setNodeCoords=function(current,nodes,index,layout,sectorId,parentId){layout.addToSector(current,
sectorId++,parentId);for(var i=0;i<current.edges.length;i++){var id=current.edges[i].id;var nodeIndex=index[id];var next=nodes[nodeIndex];setNodeCoords(next,nodes,index,layout,sectorId,current.id)}};var activator=function(model){model.initialize.subscribe(function(message){if(sm.typeMask(message,{length:true,sort:"function"})===null)return function(message){var root=new sm.type.NamedValue("sm.flowchart","root",null);model.get.publish(root);var paper=new sm.type.NamedValue("sm.raphaeljs","paper",null);
model.get.publish(paper);format(message,root.value,new Layout(paper));return false}})};ontology.registerActivator(activator,["sm.channels","sm.nodes","sm.raphaeljs"]);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.nodes' ontology: "+error.message+"\n"+error.stack))}})(smallmachine,jQuery,Raphael);(function(sm,raphael){var TITLE="sm.raphaeljs";var ontology=new sm.Ontology(TITLE);ontology.addTerm("set");var activator=function(model){var percentagePattern=/^(\d+)%?$/;var svgHelper=function(elementId,width,height,asyncResult){var element=document.getElementById(elementId+"");if(typeof element==="undefined"||!element)throw new Error("The specified element ID could not be found: "+elementId);var w=window,d=document,e=d.documentElement,g=d.getElementsByTagName("body")[0],x=w.innerWidth||e.clientWidth||
