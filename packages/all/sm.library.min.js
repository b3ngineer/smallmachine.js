...
0;i<b.dependencies.length;i++)if(a.title==b.dependencies[i])return-1;return 0})}}return ontology.getModel(behaviors)};core.alsoBehavesLike=function(a,b){if(typeof a==="undefined"||typeof b==="undefined")core.error(new Error("Cannot mixin with an undefined object"));for(var p in b){if(!b.hasOwnProperty(p))continue;if(p.indexOf("_")==0)continue;if(typeof a[p]!=="undefined"){var currentBehaviorIsVirtual=typeof a[p].prototype!=="undefined"?a[p].prototype.virtual:false;var newBehaviorIsVirtual=typeof b[p].prototype!==
"undefined"?b[p].prototype.virtual:false;if(currentBehaviorIsVirtual&&newBehaviorIsVirtual)continue;else if(currentBehaviorIsVirtual){a[p]=b[p];continue}else if(newBehaviorIsVirtual)continue}else a[p]=b[p]}if(typeof b.prototype!=="undefined"){if(typeof a.prototype==="undefined")a.prototype={};core.alsoBehavesLike(a.prototype,b.prototype)}};core.error=function(Error,handler){if(typeof handler!=="undefined"&&typeof handler.handleError==="function")handler.handleError(Error);else if(typeof core.handleError===
...
[];var alreadyRelatesTo=false;for(var i=0;i<this._relatesTo.length;i++)if(this._relatesTo[i]._id==Term._id){alreadyRelatesTo=true;break}if(!alreadyRelatesTo)this._relatesTo.push(Term)};_Term.prototype.getType=function(){return"[object Term]"};_Term.prototype.ofType=function(type){return type==="Term"||typeof type.getType==="function"&&type.getType()===this.getType()};core.Behavior=function(title){this.title=title;return this};core.Ontology=function(title){this.title=title;this._inferencer=new Inferencer;
this._activators=[];this._term=function(value){this._id=function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(c){var r=Math.random()*16|0,v=c=="x"?r:r&3|8;return v.toString(16)})}();if(typeof value._value!=="undefined"&&typeof value._type!=="undefined"){this._value=value._value;this._type=value._type}else{this._value=value;this._type=null}return this};return this};core.Ontology.prototype.getType=function(){return"[object Ontology]"};core.Ontology.prototype.addTerm=function(value){if(typeof this[value]!==
"undefined")return this;var Term=this._term;Term.prototype.relate=_Term.prototype.relate;Term.prototype.getType=_Term.prototype.getType;Term.prototype.ofType=_Term.prototype.ofType;var t=new Term(value);this[value]=new Proxy(t,this._inferencer);return this};core.Ontology.prototype.registerActivator=function(activator,dependencies){if(typeof dependencies==="undefined")dependencies=[];if(typeof activator!=="function"&&typeof activator.getType==="function"&&activator.getType()==="[object Activator]"){this._activators.push(activator);
return}this._activators.push({fn:activator,title:this.title,dependencies:dependencies,getType:function(){return"[object Activator]"}})};core.Ontology.prototype.getModel=function(behaviors){var Model=function(title){return this};var model=new Model;for(var p in this){if(!this.hasOwnProperty(p))continue;if(typeof this[p]==="undefined"||this[p]._term==="undefined")continue;if(p.indexOf("_")===0)continue;if(typeof this[p]._term!=="undefined"){model[p]=this[p]._term;if(typeof behaviors!=="undefined"){var allBehaviors=
[].concat(behaviors);for(var i=0;i<allBehaviors.length;i++)core.alsoBehavesLike(Object.getPrototypeOf(model[p]),allBehaviors[i].prototype)}}else model[p]=this[p]}this._inferencer._rules.sort(function(a,b){return a._priority-b._priority});for(var i=0;i<this._inferencer._rules.length;i++){var rule=this._inferencer._rules[i];rule._fn.apply(model,rule._args)}for(var i=0;i<this._activators.length;i++)this._activators[i].fn(model);return model};var Inferencer=function(){this._term={_value:null};this._rules=
[];return this};var Rule=function(priority,fn,args){this._fn=fn;this._args=args;this._priority=priority;return this};Inferencer.prototype.isA=function(Proxy){this._rules.push(new Rule(1,isA,[this._term._value,Proxy._term._value]));return this};Inferencer.prototype.relatesTo=function(ProxyA,ProxyB){this._rules.push(new Rule(10,relatesTo,[this._term._value,ProxyA._term._value,ProxyB._term._value]));return this};Inferencer.prototype.hasDomain=function(Proxy){this._rules.push(new Rule(3,hasDomain,[this._term._value,
Proxy._term._value]));return this};Inferencer.prototype.hasRange=function(Proxy){this._rules.push(new Rule(2,hasRange,[this._term._value,Proxy._term._value]));return this};core.alsoBehavesLike(Proxy,Inferencer);var relatesTo=function(target,termA,termB){var Term=this[target];var TermA=this[termA];var TermB=this[termB];if(TermA._type===core.CONCEPT)core.error(new Error("Cannot define a relationship with a concept type: "+TermA._value));else if(TermA._type===null)TermA._type=core.RELATIONSHIP;if(TermB._type===
core.RELATIONSHIP)core.error(new Error("Cannot create a relationship with a relationship type: "+TermB._value));else if(TermB._type===null)TermB._type=core.CONCEPT;if(Term._type===core.RELATIONSHIP)core.error(new Error("Cannot create a relationship from a relationship type: "+Term._value));else if(Term._type===null)Term._type=core.CONCEPT;if(typeof Term[termA]==="undefined"){var ctor=Term.constructor;Term[termA]=new ctor(TermA)}if(typeof Term[termA][termB]==="undefined")Term[termA][termB]=TermB;if(typeof Term[termB]===
"undefined")Term[termB]=TermB;TermB.relate(Term[termA]);for(var field in TermB){if(!TermB.hasOwnProperty(field))continue;if(TermB[field].getType&&TermB[field].getType()==="[object Term]"){if(typeof Term[termA][field]==="undefined")Term[termA][field]=TermB[field];if(typeof Term[field]==="undefined")Term[field]=TermB[field];TermB[field].relate(Term[termA])}}};var isA=function(target,termA){var Term=this[target];var TermA=this[termA];if(TermA._type===core.RELATIONSHIP)core.error(new Error("Cannot apply isA to a relationship type: "+
...
model.set.subscribe({update:function(message){return defaultStorageDelegate}});var defaultValueDelegate={update:function(message){if(typeof message.ofType==="function"&&message.ofType("NamedValue"))return function(message){model.memory.getValue(message)};return false}};model.get.subscribe({update:function(message){return defaultValueDelegate}})};ontology.registerActivator(activator);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.channels' ontology: "+
...
paper;return this};Edge.prototype.update=function(message){this.paper.path(["M",this.x1,this.y1,"L",this.x2,this.y2]);var x1=parseFloat(this.x1);var y1=parseFloat(this.y1);var x2=parseFloat(this.x2);var y2=parseFloat(this.y2);var a=x1-x2;var b=y1-y2;var cX=(x1+x2)/2;var cY=(y1+y2)/2;var textLabel=this.paper.text(cX,cY-6,this.label);var dX=x2-x1;var dY=y2-y1;var angle=Math.atan2(dY,dX)*180/Math.PI;textLabel.transform("r"+angle);return true};var getObjectNode=function(message,id){for(var i=0;i<message.length;i++)if(message[i].id===
id)return message[i];return null};var InitializerDelegate=function(model){this._model=model;this.handleError=function(Error){model.messenger.error.publish(Error)};return this};var LeafSort=function(a,b){return b.edges.length-a.edges.length};InitializerDelegate.prototype.update=function(message){if(sm.typeMask(message,{length:true,sort:"function"})!==null)sm.error(new Error("Cannot determine the message type; expecting an array (or comparable object) of node objects but received "+message),this);var paper=
new sm.type.NamedValue("sm.raphaeljs","paper",null);this._model.get.publish(paper);var edges=[];for(var i=0;i<message.length;i++){var node=new Node(message[i],paper.value);for(var j=0;j<node.edges.length;j++){var objectNode=getObjectNode(message,node.edges[j].id);if(objectNode!==null){var edge=new Edge(node,objectNode,j,paper.value);this._model.connect.publish(edge)}}this._model.insert.publish(node)}};var activator=function(model){model.initialize.subscribe(function(message){if(sm.typeMask(message,
{length:true,sort:"function"})===null)return function(message){message.sort(LeafSort);return false}});var delegate=new InitializerDelegate(model);model.initialize.subscribe(function(message){return delegate});model.connect.subscribe(function(message){return message});model.insert.subscribe(function(message){return message})};ontology.registerActivator(activator,["sm.channels","sm.raphaeljs"]);try{sm.ontology.extendedBy(ontology)}catch(error){sm.error(new Error("Could not extend the smallmachine.ontology property with the 'sm.nodes' ontology: "+
